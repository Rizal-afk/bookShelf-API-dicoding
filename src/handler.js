...
const addBookHandler = (request, h) => {
    ...
    //nilai properti readPage lebih besar dari pageCount
    ...
    //Server gagal input buku karena generic error (masalah umum)
    const response = h.response({
        status : "error",
...
const getAllBooksHandler = (request,h) => {
    ...
    if(name){
         //terdapat query name, fungsi RegExp digunakan untuk melakukan pencocokkan.
         //flag i ('i') digunakan untuk non-case sensitive
        const nameRegex = new RegExp(name, 'i');
        const filterName = books.filter((book) => nameRegex.test(book.name));
        const response = h.response({
            status : "success",
            data: {
